...
def main():
    ...
    fname = GetType(args.data_path) + ".{}.w2s.tfidf.jsonl".format(sentaspara)
    ...
    with open(args.data_path) as f:
        for line in f:
            ...
            if sentaspara == 'sent':
                ...
                save_dict = {}
                for i, para in enumerate(sents):
                    para = [para]
                    cntvector, tfidf_weight = get_tfidf_embedding(para)
                    id2word = {}
                    for w, tfidf_id in cntvector.vocabulary_.items():   # word -> tfidf matrix row number
                        id2word[tfidf_id] = w
                    tfidfvector = compress_array(tfidf_weight, id2word)
                    save_dict[i] = tfidfvector[0]
